...
class ProgressesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create progress" do
    assert_difference("Progress.count") do
      post progresses_url, params: { progress: { date: @progress.date, habit_id: @progress.habit_id, status: @progress.status } }
    end
  ...
  test "should update progress" do
    patch progress_url(@progress), params: { progress: { date: @progress.date, habit_id: @progress.habit_id, status: @progress.status } }
